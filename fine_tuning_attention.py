...
import argparse
import wandb
from train_attention import Lang,readLangs,prepareData,indexesFromWord,zeroPadding,binaryMatrix,inputVar,outputVar,batch2TrainData,get_activation,get_optimizer,maskNLLLoss,mask_accuracy,evaluate,evaluateIters
from train_attention import AttnDecoderRNN,EncoderRNN
...
def train_wandb(config = default_config):
    ...
    checkpoint = torch.load('attention_models/%s'%(model_pth))
