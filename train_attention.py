...
def indexesFromWord(lang,word ):
    return [lang.letter2index[letter] for letter in word] + [EOS_token]
...
class AttnDecoderRNN(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, output_size, n_layers=1, dropout=0.1,cell_type = 'lstm',bidirectional = False,activation = 'relu'):
        ...
        if cell_type=='gru':
            self.model = nn.GRU(hidden_size, hidden_size, n_layers, dropout=(0 if n_layers == 1 else dropout))
        if cell_type=='lstm':
            self.model = nn.LSTM(hidden_size, hidden_size, n_layers, dropout=(0 if n_layers == 1 else dropout))
...
def trainIters(model_name,pairs,encoder,decoder,encoder_optimizer,decoder_optimizer,encoder_n_layers,decoder_n_layers,save_dir,n_iterations,batch_size,print_every,save_every,clip,teacher_forcing_ratio,input_lang,output_lang,val_pairs):
    ...
    for iteration in range(start_iteration,n_iterations+1):
        ...
        if iteration % print_every == 0:
            ...
            },os.path.join('attention_models','%f_accuracy_%s_model.pth'%(print_accuracy_avg*100,model_name)))
            print_loss = 0
            print_accuracy = 0
        end = time.time()
...
def train_wandb(config = default_config):
    ...
    decoder_embedding_size = config.hidden_size
